...
void main() {
    ...
    // define temp coor to resolve TextureCoor Problem .
    vec2 tempCoord;

    // 纹理
    #ifdef textureFlag
    // 属性变量不可修改
    tempCoord = vec2(a_texCoord0.x, 1.0 - a_texCoord0.y);
    #endif

    // resolve reverse UV flag
    #if defined(reverseUVFlag) && defined(textureFlag)
    tempCoord = vec2(a_texCoord0.x, a_texCoord0.y);
    #endif
    ...
    v_diffuseUV = u_diffuseUVTransform.xy + tempCoord * u_diffuseUVTransform.zw;
    #endif//diffuseTextureFlag

    //自发光纹理,以及纹理变化
    #ifdef emissiveTextureFlag
    v_emissiveUV = u_emissiveUVTransform.xy + tempCoord * u_emissiveUVTransform.zw;
    #endif//emissiveTextureFlag

    //镜面纹理
    #ifdef specularTextureFlag
    v_specularUV = u_specularUVTransform.xy + tempCoord * u_specularUVTransform.zw;
