...
public class XDatePickerSpinnerDelegate extends XDatePicker.AbstractXDatePickerDelegate {
    ...
    public XDatePickerSpinnerDelegate(XDatePicker xDatePicker, Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, acu.l.XDatePicker, i, i2);
        boolean z = obtainStyledAttributes.getBoolean(acu.l.XDatePicker_dp_spinnersShown, true);
        int i3 = obtainStyledAttributes.getInt(acu.l.XDatePicker_dp_startYear, DEFAULT_START_YEAR);
        int i4 = obtainStyledAttributes.getInt(acu.l.XDatePicker_dp_endYear, DEFAULT_END_YEAR);
        String string = obtainStyledAttributes.getString(acu.l.XDatePicker_dp_minDate);
        String string2 = obtainStyledAttributes.getString(acu.l.XDatePicker_dp_maxDate);
        int resourceId = obtainStyledAttributes.getResourceId(acu.l.XDatePicker_dp_xDatePickerLayout, acu.i.x_date_picker_layout);
        ...
        XNumberPicker.OnValueChangeListener onValueChangeListener = new XNumberPicker.OnValueChangeListener() { // from class: com.xiaopeng.xui.widget.datepicker.-$$Lambda$XDatePickerSpinnerDelegate$bvC71pUOc2APz4A8E3c-YsjbyLM
            ...
            public final void onValueChange(XNumberPicker xNumberPicker, int i5, int i6) {
                XDatePickerSpinnerDelegate.lambda$new$0(XDatePickerSpinnerDelegate.this, xNumberPicker, i5, i6);
            }
        };
        this.mSpinners = (LinearLayout) this.mXDelegator.findViewById(acu.g.pickers);
        reorderSpinners(new XNumberPicker[]{(XNumberPicker) this.mXDelegator.findViewById(acu.g.picker1), (XNumberPicker) this.mXDelegator.findViewById(acu.g.picker2), (XNumberPicker) this.mXDelegator.findViewById(acu.g.picker3)});
    ...
    public static /* synthetic */ void lambda$new$0(XDatePickerSpinnerDelegate xDatePickerSpinnerDelegate, XNumberPicker xNumberPicker, int i, int i2) {
        xDatePickerSpinnerDelegate.mTempDate.setTimeInMillis(xDatePickerSpinnerDelegate.mCurrentDate.getTimeInMillis());
        if (xNumberPicker == xDatePickerSpinnerDelegate.mDaySpinner) {
            int actualMaximum = xDatePickerSpinnerDelegate.mTempDate.getActualMaximum(5);
            if (i == actualMaximum && i2 == 1) {
                xDatePickerSpinnerDelegate.mTempDate.add(5, 1);
            } else if (i == 1 && i2 == actualMaximum) {
                xDatePickerSpinnerDelegate.mTempDate.add(5, -1);
            } else {
                xDatePickerSpinnerDelegate.mTempDate.add(5, i2 - i);
            }
        } else if (xNumberPicker == xDatePickerSpinnerDelegate.mMonthSpinner) {
            if (i == 11 && i2 == 0) {
                xDatePickerSpinnerDelegate.mTempDate.add(2, 1);
            } else if (i == 0 && i2 == 11) {
                xDatePickerSpinnerDelegate.mTempDate.add(2, -1);
            } else {
                xDatePickerSpinnerDelegate.mTempDate.add(2, i2 - i);
            }
        } else if (xNumberPicker == xDatePickerSpinnerDelegate.mYearSpinner) {
            xDatePickerSpinnerDelegate.mTempDate.set(1, i2);
        } else {
            throw new IllegalArgumentException();
        }
        xDatePickerSpinnerDelegate.setDate(xDatePickerSpinnerDelegate.mTempDate.get(1), xDatePickerSpinnerDelegate.mTempDate.get(2), xDatePickerSpinnerDelegate.mTempDate.get(5));
        xDatePickerSpinnerDelegate.updateSpinners();
        xDatePickerSpinnerDelegate.notifyDateChanged();
    }
    ...
    private void updateSpinners() {
        ...
        while (i < this.mCurrentDate.getActualMaximum(5)) {
            int i2 = i + 1;
            strArr[i] = this.mContext.getResources().getString(acu.j.x_date_picker_day, Integer.valueOf(i2));
            i = i2;
        }
        ...
        for (int i3 = 0; i3 < strArr2.length; i3++) {
            strArr2[i3] = this.mContext.getResources().getString(acu.j.x_date_picker_year, Integer.valueOf(this.mYearSpinner.getMinValue() + i3));
        }
